...
A logistic regression model for build using the scikit-learn library
...
- src: Source code containing the package "logreg_model_wrapper" with model wrapper functions
- Makefile: Build file
...
- Switch to the `tests/context/<interface>` directory and then use
    - `make docker` to build the model container
    - The image name will be `scikit-logreg-model` (`scikit-logreg-model-grpc` if using grpc interface)
...
- The latter two commands require the following project structure
    ...
    ├─ nodes
    |  ├─ scikit-logreg-model // This repository
