...
A tool for computing common uncertainty estimation metrics for classification and regression models build using the Uncertainty Toolbox library
...
- src: Source code containing the package "uct_wrapper" with tool wrapper functions
- Makefile: Build file
...
- Switch to the `tests/context/<interface>` directory and then use
    ...
    - The image name will be `uncertainty-toolbox-metrics` (`uncertainty-toolbox-metrics-grpc` if using grpc interface)
...
- The latter two commands require the following project structure
    ...
    ├─ nodes
    |  ├─ uncertainty-toolbox-metrics // This repository
